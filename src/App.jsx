...
If the math does NOT check out, you have likely misread a digit. GO BACK and re-read the numbers from the receipt image again, character by character. The receipt's PRINTED numbers are ALWAYS correct — the receipt was generated by a computer. Your job is to read them accurately, not to fix them.

DO NOT "correct" receipt values based on math. If 45.23 × 2.829 = 127.96 on the receipt but your calculation gives 127.95, the receipt is right — use 127.96. The receipt's numbers are the ground truth.
...
- CROSS-CHECK EVERY PRODUCT: litres × pricePerLitre ≈ cost. If it doesn't match, RE-READ the digits from the image — do NOT invent corrected values.
...
- CONFIDENCE: "high" = clear image, all values readable. "medium" = some blurry/uncertain digits. "low" = very blurry or unreadable. Always list specific digit uncertainties in issues.
- REMEMBER: The receipt was printed by a computer — its numbers are ALWAYS internally consistent. Your ONLY job is to read them accurately in the exact order they appear, left to right. Never rearrange digits.`;
...
function normalizeReceiptData(data) {
  ...
  // We ONLY fill in genuinely MISSING values, never override AI-read values.
  ...
  data.lines = data.lines.map((line, idx) => {
    ...
    // If all 3 exist, verify consistency — but ONLY FLAG, do NOT auto-correct
    // The AI was told to read digits left-to-right exactly as printed.
    // If the math is off, it means the AI misread a digit — flagging helps human review.
    if (line.litres && line.cost && line.cost > 0 && (line.pricePerLitre || ppl)) {
      ...
      if (diff > tolerance && diff > 0.50) {
        // Math doesn't check out — flag for human review but DO NOT override AI-read values
        ...
        // ONLY correct if the discrepancy is very large (>15%) suggesting a major misread
        // like reading pump meter total instead of quantity, or cents vs dollars confusion
        if (diff > line.cost * 0.15 && diff > 5.00) {
          ...
          if (priceReasonable && Math.abs(calcPrice - lp) > 0.05) {
            // Cost and litres both look like real receipt values, price was likely from wrong line
            ...
            // Litres is suspiciously large compared to cost — probably read pump meter total
            const calcLitres = parseFloat((line.cost / lp).toFixed(2));
            line._originalLitres = line.litres;
            line.litres = calcLitres;
            line._corrected = true;
            data._mathIssues.push(`Line ${idx+1}: litres ${litres} looks like pump meter reading — adjusted to ${calcLitres}`);
          }
          // Otherwise: leave values as-is and let the human review the flag
        }
      }
    }
    return line;
